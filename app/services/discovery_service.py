...
def fetch_json(url: str, params: dict[str, str] | None = None) -> object:
    try:
        with httpx.Client(timeout=20, follow_redirects=True, headers={"User-Agent": "JobSniffing/0.2"}) as client:
...
def provider_request(request: DiscoveryRequest) -> tuple[str, dict[str, str] | None]:
    ...
    if not _IDENTIFIER.fullmatch(identifier):
        raise DiscoveryError("identifier may contain only letters, numbers, underscores, and hyphens")
...
def discover(request: DiscoveryRequest, fetcher: FetchJson = fetch_json) -> list[JobPostingIn]:
    ...
    query, location = request.query.strip().lower(), request.location.strip().lower()
    result = []
    for job in jobs:
