...
def upsert_job(
    ...
    score = score_text(
        ...
        job.description,
        job.location,
        settings.preferred_terms,
        settings.excluded_terms,
    )
    initial = (
        ApplicationStatus.BLOCKED
        if is_blacklisted(job.company, settings)
        else ApplicationStatus.SCORED
    )
    conn.execute(
        """
        ...
        ON CONFLICT(source, external_id) DO UPDATE SET
          ...
          status=CASE
            WHEN excluded.status='blocked'
              AND job_postings.status NOT IN ('submitted', 'blocked')
            THEN 'blocked'
    ...
    *,
    ...
    return list(
        conn.execute(
            ...
            "ORDER BY score DESC,updated_at DESC,id DESC LIMIT ?",
            params,
        )
    )
...
def rescore_all(
    ...
    settings = settings or load_scoring_settings(conn)
    rows = conn.execute(
        "SELECT id,title,description,location,company,status FROM job_postings"
    ).fetchall()
    for row in rows:
        score = score_text(
            row["title"],
        ...
        target = current
        if (
            current not in {ApplicationStatus.SUBMITTED, ApplicationStatus.BLOCKED}
            and is_blacklisted(row["company"], settings)
        ):
            target = ApplicationStatus.BLOCKED
        conn.execute(
            "UPDATE job_postings SET score=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (score, target.value, row["id"]),
        )
    return len(rows)
...
def stats(conn: sqlite3.Connection) -> dict[str, object]:
    ...
    by_status = {
        ...
        for row in conn.execute(
            "SELECT status,COUNT(*) count FROM job_postings GROUP BY status"
        )
    }
    average = conn.execute(
        "SELECT COALESCE(ROUND(AVG(score),1),0) FROM job_postings"
    ).fetchone()[0]
